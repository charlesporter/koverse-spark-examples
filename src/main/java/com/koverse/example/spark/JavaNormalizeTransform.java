...
/**
 * Adapted from JavaWordCounterTransform.
 */
public class JavaNormalizeTransform extends JavaSparkTransform {
  ...
  protected JavaRDD<SimpleRecord> execute(JavaSparkTransformContext context) {
    ...
    JavaRDD<SimpleRecord> input = context.getInputCollectionRdds().get(inputCollectionId);
    ...
    final JavaNormalizer normalizer = new JavaNormalizer(fieldName, "['\".?!,:;\\s]+");

    JavaRDD<SimpleRecord> inputRecordsRdd = normalizer.createIds(input);


    final Map<String, String> dictionary = normalizer.createDictionary(inputRecordsRdd);
    normalizer.broadcastDictionary(context.getJavaSparkContext(), dictionary);
    final JavaPairRDD<String, Map<String, Double>> documentVectors =
        normalizer.createVectors(inputRecordsRdd);

    return normalizer.vectorsToSimpleRecords(documentVectors);
  ...
  public String getDescription() {
    return "Create normalized vectors for a corpus";
  }
}
